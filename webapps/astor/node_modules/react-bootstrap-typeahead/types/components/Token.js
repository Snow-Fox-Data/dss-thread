...
const Token = ({ readOnly, ...props }) => {
    const { ref, ...tokenProps } = useToken(props);
    return !props.disabled && !readOnly && isFunction(tokenProps.onRemove) ? (React.createElement(InteractiveToken, { ...props, ...tokenProps, ref: ref })) : (React.createElement(StaticToken, { ...props }));
};
