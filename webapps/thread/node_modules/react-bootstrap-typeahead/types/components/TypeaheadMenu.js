...
const defaultProps = {
    ...
    renderMenuItemChildren: (option, props) => (React.createElement(Highlighter, { search: props.text }, getOptionLabel(option, props.labelKey))),
};
const TypeaheadMenu = (props) => {
    const { labelKey, newSelectionPrefix, options, paginationText, renderMenuItemChildren, ...menuProps } = props;
    const renderMenuItem = (option, position) => {
        ...
        if (getOptionProperty(option, 'customOption')) {
            return (React.createElement(MenuItem, { ...menuItemProps, className: "rbt-menu-custom-option", key: position, label: label },
                newSelectionPrefix,
                React.createElement(Highlighter, { search: menuProps.text }, label)));
        }
    ...
    return React.createElement(Menu, { ...menuProps }, options.map(renderMenuItem));
};
